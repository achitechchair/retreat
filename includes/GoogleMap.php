	...
	*/
    ...
    function geoGetCoords($address,$depth=0) {
        switch($this->lookup_service) {
            case 'GOOGLE':
                $_url = sprintf('https://maps.googleapis.com/maps/api/geocode/json?sensor=%s&address=%s&key',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", rawurlencode($address));
                $_result = false;

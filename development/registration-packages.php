		...
		<div class="inner_container">
         ...
         <form name="reg_frm" id="reg_frm" method="post" action="<?php echo MAIN_WEBSITE_URL?>/registration-packages">
            <div class="register_area">
               ...
               <?php
						$old_order = "";
                  $sql_package = "SELECT * FROM `tbl_registration_packages` WHERE `status`='Active' AND `mark_for_deleted`='No' ORDER BY `display_order_segregate` ASC, `display_order` ASC";
                  $res_package = $db->get($sql_package);
                  while($row_package = $db->fetch_array($res_package))
                  {							
               ?>
               <?php
							if($old_order!=$row_package['display_order_segregate'] && empty($old_order) == FALSE)
							{
					?>  
                <hr>          
               <?php
							}
							$old_order=$row_package['display_order_segregate'];
               ?>  
               
               <div class="input_radio"><input name="pack_name[]" id="pack_<?php echo $row_package['reg_pack_id'];?>" type="checkbox" value="<?php echo $row_package['reg_pack_id'];?>" class="all_package all_pack_<?php echo $row_package['room_for_nights'];?>" alt="<?php echo $row_package['room_for_nights'];?>" required />  <?php echo $f->getValue($row_package['package_name']);?>&nbsp;-&nbsp;$<?php echo round($row_package['package_price']);?><?php if($row_package['package_price_old'] > 0) echo " <span class='old_price'>$".round($row_package['package_price_old'])."</span>";?> <a href="" data-toggle="modal" data-target="#<?php echo $row_package['reg_pack_id'];?>">View Details</a></div>
...
$(document).ready(function(){	
	...
	$('.all_package').click(function() {
		...
		var room_for_nights = $(this).attr("alt"); 	
		let cnt = '';	
		...
		var pack_name_array = []; 
  		...
  		$("input:checkbox[name='pack_name[]']:checked").each(function(){    
			...
			cnt+= parseInt($(this).attr("alt"));
  		});
		
  		var all_pack_select = pack_name_array.join(",");
		
		
		// if all packages enabled ------------------------------------------------------------
		if(all_pack_select=='')
		{
			$('.all_package').each(function() {					
				$(this).attr("checked", false);		
				$(this).attr("disabled", false);
			});
			
		}else{
		// end if all packages enabled --------------------------------------------------------
		
		// same package room are desabled -------------------------------------------------------
		if($(this).attr("alt")!=0)
		{
				$("input:checkbox[name='pack_name[]']").each(function(){
					var this_room_for_nights = $(this).attr("alt");
					
					if(room_for_nights!='' && room_for_nights!=this_room_for_nights)
					{					
						$(".all_package[alt='"+this_room_for_nights+"']").attr("disabled", true);
						$(".all_package[alt='0']").attr("disabled", false);	
					}
								
				});	
			}
		}	
		// end same package room are desabled --------------------------------------------------

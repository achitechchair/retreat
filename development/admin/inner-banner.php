				...
				<table class="table table-bordered <?php if($records > 0) echo ' data-table';?>">
					...
					<tbody>
						...
						<!--<div id="myAlert<?php echo $row['inner_banner_id'];?>" class="modal hide">
							...
							<div class="modal-footer"> <a class="btn btn-primary" href="<?php echo CP;?>?index=List&action=Delete&Id=<?php echo $inner_banner_id;?>&lang_name=<?php echo $f->getValue($row['language_name']);?>">Confirm</a> <a data-dismiss="modal" class="btn" href="#">Cancel</a> </div>
						</div>-->
					<?php 
							}
						} else { 

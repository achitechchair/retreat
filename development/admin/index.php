<?php
	...
	{
		$username = $f->XssPOST($_POST['username']);
		$password = $f->XssPOST($_POST['password']);
		...
		if($db->num_rows($res) > 0):
			...
			{
				$_SESSION['_admin_id'] = $row['user_id'];
				$_SESSION['_admin_fname'] = $f->getValue($row['fname'])." ".$f->getValue($row['lname']);				
				$_SESSION['_admin_user_type'] = 'AdminUser';
			}		
...
<div id="loginbox">
	<form action="<?php echo CP;?>" method="post" name="frmLogin" id="basic_validate" novalidate>
		...
		<div class="control-group">
			<div class="controls">
				<div class="main_input_box"> <span class="add-on bg_lg"><i class="icon-user"> </i></span>
					<input name="username" type="text" id="username" value="<?php echo $_POST['username'];?>" class="required" placeholder="User Name" />
				</div>
			</div>
		</div>
		<div class="control-group">
		...
		<span class="pull-right">
			...
			<input name="redirect" type="hidden" id="redirect" value="<?php echo $return = (empty($_GET['redirectTo'])==false) ? $_GET['redirectTo'] : $_POST['redirect'];?>" />

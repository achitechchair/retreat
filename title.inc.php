<?php 
	...
	{
		$seo_page_title = (empty($seo_row[0]) == FALSE) ? ($f->getValue($seo_row[0])) : ('');
	}
	if(empty($seo_meta_keywords) == TRUE)
	{
		$seo_meta_keywords = (empty($seo_row[1]) == FALSE) ? ($f->getValue($seo_row[1])) : ('');
	}
	if(empty($seo_meta_description) == TRUE)
	{
		$seo_meta_description = (empty($seo_row[2]) == FALSE) ? ($f->NoEnter($f->getValue($seo_row[2]), FALSE)) : ('');
	}	

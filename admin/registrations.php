...
<div id="content"> 
	...
	if($index == 'List')
	{
		...
		$form_date = (empty($_GET['form_date']) == FALSE) ? ($_GET['form_date']) : ('');
		$to_date = (empty($_GET['to_date']) == FALSE) ? ($_GET['to_date']) : ('');
		$sctext = (empty($_GET['sctext']) == FALSE) ? ($_GET['sctext']) : ('');
		$pay_mode = (empty($_GET['pay_mode']) == FALSE) ? ($_GET['pay_mode']) : ('');
		...
		{			
			$sql.= " AND (a.`ref_number`='".$sctext."' OR b.`email_id` LIKE '".$sctext."%' OR b.`phone` LIKE '".$sctext."%') ";
		}	
	...
	<table class="table" cellpadding="0" cellspacing="0" border="0">
   	<tr>
          ...
          <td width="150">
             ...
             <option value="all"<?php if(empty($search_by_status) == FALSE && ($search_by_status == 'all' || $search_by_status == '')){ echo ' selected';}?>>Pay Mode</option>
				 <option value="Zelle"<?php if(empty($search_by_status) == FALSE && $search_by_status == 'Zelle'){ echo ' selected';}?>>Zelle</option>
             <option value="Check"<?php if(empty($search_by_status) == FALSE && $search_by_status == 'Check'){ echo ' selected';}?>>Check</option>
             <option value="PayPal"<?php if(empty($search_by_status) == FALSE && $search_by_status == 'PayPal'){ echo ' selected';}?>>PayPal</option>          
            </select>
          </td>
          <td width="100"> 
			...
			<input type="hidden" name="btnseq" value="ok">	
				<table class="table table-bordered <?php if($records > 0) echo ' data-table';?>" width="100%">
					...
					<tbody>
						<?php
							if($records > 0) 
							{
								while($row = $db->fetch_array($res)) 
								{
									...
									if($pay_status == 'Partially Paid' || $pay_status == 'Pending' || $pay_status == 'Canceled' || $pay_status == 'Unpaid')
									{
										$str = "<b>".$pay_status."</b>";
                     ...
                     <td><div align="center"><a style="color: <?php echo $color;?>;" title="<?php echo $title;?>" href="<?php echo $href;?>" class=""><?php echo $str;?></a></div></td>
                     <?php 
                     ...
                     <td><div align="center">
						...
						<div id="myAlert<?php echo $row['reg_id'];?>" class="modal hide">
							...
							<div class="modal-footer"> <a class="btn btn-primary" href="<?php echo CP;?>?index=List&action=Delete&Id=<?php echo $reg_id;?>&file=">Confirm</a> <a data-dismiss="modal" class="btn" href="#">Cancel</a> </div>
						</div>
					<?php 
							}
						} else { 
	...
	<?php 
			...
			{	
				...
				if($pay_status_post == $pay_status)
				{
					...
					/*if($pay_status_post == 'Canceled')
					{
						$ref_number = substr($ref_number,1);
						$data['ref_number'] = "C".$ref_number;
						$data['cancel_dt'] = date('Y-m-d H:i:s');
						$data['status'] = 'Cancel';
					}*/
   	...
   	<input type="hidden" name="reg_id" value="<?php echo $reg_id;?>">
		...
		<div class="row-fluid">
			<div class="widget-box">
				...
				<div class="widget-content nopadding">	 
            	<?php 
						$sql_info = "SELECT `first_name`, `last_name` FROM `tbl_reg_members_in_your_party` WHERE `reg_id`=".$Id." ORDER BY `reg_members_in_your_party_id` ASC LIMIT 0, 1";
						$res_info = $db->get($sql_info);
						$row_info = $db->fetch_array($res_info);
					?>
            	<div class="control-group comment_block">						
						<div class="controls">
							<b><?php echo $f->getValue($row_info['first_name']." ".$row_info['last_name']);?> (#<?php echo $ref_number;?>)</b>
						</div>
					</div>            
               ...
               <div class="control-group">
                        ...
                        <option value="Unpaid"<?php if($row['pay_status'] == 'Unpaid') echo ' selected';?>>Unpaid</option>
                        <?php } ?>
							</select>
						</div>
					</div>

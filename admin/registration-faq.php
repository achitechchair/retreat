...
<div id="content"> 
	...
	<div id="content-header">
		<div id="breadcrumb"> <a href="javascript:;" title="" class="tip-bottom"><i class="icon-th-large"></i> Manage Registration FAQ</a></div>
	</div>
	<!--End-breadcrumbs-->
	<?php 
	if($index == 'List')
	{
		...
		if(isset($_POST['btnseq']))
		{
			...
			while($rowQ = $db->fetch_array($resQ))
			{
				...
				{
					//echo $rowQ['prod_name'];						
					$sql0 = "SELECT * FROM ".T." WHERE `faq_id`='".$rowQ['faq_id']."'";
					$res0 = $db->get($sql0);
					...
					$SQLss = "UPDATE ".T." SET `display_order`='".$_POST["seq$v"]."' WHERE `faq_id`='".$rowQ['faq_id']."'";
					$RESss = $db->get($SQLss);
				}				
			}
		...
		{
			$Id = $_GET['Id'];
			$sql = "UPDATE `".T."` SET `status`='".$_GET['action']."' WHERE `faq_id`=".$Id;
		...
		{			
			foreach($_POST['all_check_bx'] as $val):
				$sql = "UPDATE `".T."` SET `status`='".$_GET['status']."' WHERE `faq_id`=".$val;
				$db->get($sql,__FILE__,__LINE__);							
			endforeach;
		...
		if(empty($_GET['action'])==false && $_GET['action']=='Delete')
		{
			...
			$sql = "UPDATE ".T." SET `mark_for_deleted`='Yes' WHERE `faq_id`=".$Id;
			$db->get($sql,__FILE__,__LINE__);
  ...
  ?>
	...
	<div class="widget-box">
		<div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
			<h5>List of FAQ (<?php echo $records;?>)</h5>
		</div>
			...
			<input type="hidden" name="btnseq" value="ok">	
				<table class="table table-bordered <?php if($records > 0) echo ' data-table';?>" style="width:100%;">
					<thead>
						<tr>							
							 <th width="2%">Seq.</th>							
							 <th width="81%"><div align="left">Title</div></th>	
					...
					<tbody>
						<?php
						 ...
						 ?>
						<tr>							
							<td><input name="seq<?php echo $faq_id;?>" type="text" value="<?php echo $row['display_order'];?>" size="2"  class="Sequ numeric" /></td>
							<td><?php echo $f->getValue($row['faq_title']);?></td>	
							<td><div align="center"<?php if($row['status'] == 'Inactive') echo ' style="color:#F00;"';?>><?php echo $f->getValue($row['status']);?></div></td>						
							<td><div align="center"><a href="<?php echo CP;?>?index=Edit&Id=<?php echo $faq_id;?>" class="btn btn-primary btn-mini">Edit</a> <a href="#myAlert<?php echo $faq_id;?>" data-toggle="modal" class="btn btn-danger btn-mini">Delete</a></div></td>
						</tr>
						<div id="myAlert<?php echo $row['faq_id'];?>" class="modal hide">
							...
							<div class="modal-footer"> <a class="btn btn-primary" href="<?php echo CP;?>?index=List&action=Delete&Id=<?php echo $faq_id;?>&file=<?php echo $row['cat_image'];?>">Confirm</a> <a data-dismiss="modal" class="btn" href="#">Cancel</a> </div>
						</div>
					<?php 
							}
						} else { 
	...
	elseif($index == 'Add')
	{
		...
		{					
			$faq_title = $f->setValue($_POST['faq_title']);
			
			$sql = "SELECT * FROM `".T."` WHERE `faq_title`='".$faq_title."' AND `mark_for_deleted`='No'";
			$res = $db->get($sql,__FILE__,__LINE__);
			if($db->num_rows($res) > 0) 
			{
				$msg = $f->getHtmlError('Title "'.$f->getValue($faq_title).'" already exist !!!');
			} else {	
					
				
				$data = array(
					'faq_title' => $faq_title,
					'faq_slug' => $f->slugify($_POST['faq_title']),
					'faq_desc' => $f->setValue($_POST['faq_desc']),
...
?>
			...
			<div class="widget-box">
				...
				<div class="widget-content nopadding">
					<div class="control-group">
						...
						<div class="controls">
							<input name="faq_title" type="text" id="faq_title" value="<?php echo $f->POST_VAL('faq_title');?>" class="span11 required" />								
						</div>
					</div>					
					<div class="control-group">
						<label class="control-label">Description:</label>
						<div class="controls"> 
							<textarea name="faq_desc" id="faq_desc" class="span11 required ckeditor"><?php echo $f->POST_VAL('faq_desc');?></textarea>	
						</div>
					</div>
					<div class="control-group">
					...
					<div class="widget-content nopadding">
						<div class="control-group">
							<div class="controls">
								<input name="btnCreate" id="btnCreate" type="submit" value="Add FAQ" class="btn btn-success" />
	...
	<?php 
		...
		{	
			...
			$sql = "SELECT * FROM `".T."` WHERE `faq_title`='".$faq_title."' AND `mark_for_deleted`='No' AND `faq_id`<>'".$Id."'";
			$res = $db->get($sql,__FILE__,__LINE__);
			if($db->num_rows($res) > 0) 
			{
				$msg = $f->getHtmlError('Title "'.$f->getValue($faq_title).'" already exist !!!');
				...
				$db->update(T, $data, "faq_id", $Id);
				$msg = $f->getHtmlMessage("Record has been successfully updated");
			}
		}		
		
		$sql = "SELECT * FROM `".T."` WHERE `faq_id`=".$Id;
		$res = $db->get($sql,__FILE__,__LINE__);
			...
			<div class="widget-box">
				...
				<div class="widget-content nopadding">								
					...
					<div class="control-group">
						<label class="control-label">Title:</label>
						<div class="controls">
							<input name="faq_title" type="text" id="faq_title" value="<?php echo $f->getValue($row['faq_title']);?>" class="span11 required" />								
						</div>
					</div>					
					<div class="control-group">
						<label class="control-label">Description:</label>
						<div class="controls"> 
							<textarea name="faq_desc" id="faq_desc" class="span11 required ckeditor"><?php echo $f->getValue($row['faq_desc']);?></textarea>	
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">Status:</label>
					...
					<div class="widget-content nopadding">
						<div class="control-group">
							<div class="controls">
								<input name="btnEdit" id="btnEdit" type="submit" value="Modify FAQ" class="btn btn-success" />
								<input type="button" value="Cancel / Back" class="btn btn-warning" onclick="javascript:window.document.location.href='<?php echo CP.'?index=List';?>';" />
							</div>
						</div>
					</div>
